...
const prepColors = (input?: string[]) => {
  const base = (input && input.length
    ? input
    : ["#A6C8FF", "#5227FF", "#FF9FFC"]).slice(0, MAX_COLORS);
...
void mainImage(out vec4 o, vec2 C) {
  ...
  vec3 colr = sqrt(tanhv(max(O.rgb * uGlow - vec3(0.04, 0.04, 0.04), 0.0)));
...
export function Lightfall({
  ...
  return (
    <div
      ...
      style={mixBlendMode ? { ...style, mixBlendMode } : style}
    />
  );
}